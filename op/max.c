...
void op_test_max_run(int batch_size)
{
    // 准备测试数据
    srand(time(NULL));
    ...
    // FIXME: use array instead of malloc
    // float input_a[batch_size];
    // float input_b[batch_size];
    ...
    // FIXME: free will cause error, check later
    free(input_a);
    free(input_b);
    free(output);
