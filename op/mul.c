...
void op_test_mul_run(int batch_size)
{
    ...
    printf("Test scalar\n");
    int64_t start = clock();
    xnn_f32_vmul_minmax_ukernel__scalar_u8(batch_size * sizeof(float), input_a, input_b, output, &params);
    int64_t end = clock();
    printf("Time: %f ms\n", (end - start) * 1.0 / CLOCKS_PER_SEC * 1000);
    // 测试rvv
    printf("Test rvv\n");
    start = clock();
    xnn_f32_vmul_minmax_ukernel__rvv_u8v(batch_size * sizeof(float), input_a, input_b, output, &params);
    end = clock();
    printf("Time: %f ms\n", (end - start) * 1.0 / CLOCKS_PER_SEC * 1000);
    printf("-------------------------------------\n");
